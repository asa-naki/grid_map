...
void InterpolationDemo::runDemo()
{
  ...
  interpolateInputMap(
    dataSparseMap_, interpolationMethods.at(interpolationMethod_),
    &interpolatedMap_);
...
void InterpolationDemo::printStatistics(const Statistics & stats) const
{
  ...
  for (auto world = worlds.cbegin(); world != worlds.cend(); ++world) {
    ...
    for (auto method = interpolationMethods.cbegin();
      method != interpolationMethods.cend(); ++method)
    {
      Statistic s = stats.at(world->first).at(method->first);
      printf(
        "Method: %-20s  Mean absolute error: %-10f     "
        "max error: %-10f  avg query duration: %-10f microsec \n",
